...
1. **Angular Version Alignment**: Always align with the project's Angular version (v20 Standalone in this project). Use modern standalone architecture patterns rather than legacy `NgModules`.
2. **Reactivity & State Management**: Strictly use Angular **Signals** (`signal()`, `computed()`, `effect()`) for all client-side reactive variables, coordinate states, and screen changes. Avoid legacy RxJS observables or manual subscriptions for simple local state management.
3. **Template Control Flow**: Strictly adopt the modern Angular template control flow (`@if`, `@for`, `@switch`) to handle conditional rendering and lists. Avoid legacy directives (`*ngIf`, `*ngFor`, `*ngSwitch`).
4. **Dependency Injection**: Prefer modern functional dependency injection via `inject(Service)` instead of legacy constructor injection where applicable, especially in standalone components, custom resolvers, or route guards.
5. **Modern Component API**: 
   - Use Signal-based inputs: `input()` or `input.required()` instead of `@Input()`.
   - Use two-way state synchronization via `model()` inputs where applicable.
   - Use modern Signal-based outputs: `output()` instead of `@Output()`.
6. **Component Styling**: Write clean, responsive Vanilla CSS/SCSS scoped strictly to the component (`styleUrls: ['...scss']`) rather than bloating global styles. Keep layouts touch-friendly, mobile-safe, and free from scrolling collisions.
7. **Accessibility (a11y)**: Add explicit descriptive `aria-*` tags, appropriate heading levels (`h1..h6`), and unique descriptive `id` attributes on all dynamic touch targets (e.g. crop handles, modal toggles) to ensure robust accessibility and testability.
8. **Build Verification**: Once you finish generating or modifying code, proactively run the production builder (`npm run build`) and lint checks (`npm run lint`). Resolve any compiler errors or style warnings before concluding your work. Never skip this step.

---

## 2. Standard Angular Developer Guidelines
